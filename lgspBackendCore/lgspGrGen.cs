...
namespace de.unika.ipd.grGen.lgsp
{
    ...
    public class LGSPGrGen
    {
        ...
        private IEnumerable<SyntaxTree> GetModelSyntaxTreeList(CompileConfiguration cc)
        {
            String modelName = Path.GetFileNameWithoutExtension(cc.modelFilename);
            var modelFileText = File.ReadAllText(cc.modelFilename);
            var treeList = new List<SyntaxTree>() { CSharpSyntaxTree.ParseText(modelFileText,null,cc.modelFilename) };
            if (cc.modelStubFilename != null)
            {
                var modelStubFileText = File.ReadAllText(cc.modelStubFilename);
                treeList.Append(CSharpSyntaxTree.ParseText(modelStubFileText,null,cc.modelStubFilename));
            }
            var externalFuncFilename = cc.destDir + modelName + "ExternalFunctions.cs";
            if (File.Exists(externalFuncFilename))
            {
                var externalFuncFileText = File.ReadAllText(externalFuncFilename);
                treeList.Append(CSharpSyntaxTree.ParseText(cc.modelStubFilename, null, externalFuncFileText));
            }
            return treeList;
        }

        private static IEnumerable<SyntaxTree> GetIntermediateSyntaxTreeList(string actionsFilename)
        {
            var actionsFileText = File.ReadAllText(actionsFilename);
            var parseOpts = CSharpParseOptions.Default.WithPreprocessorSymbols(new[] { "INITIAL_WARMUP" });
            var treeList = new List<SyntaxTree>() { CSharpSyntaxTree.ParseText(actionsFileText, parseOpts, actionsFilename) };
            return treeList;
        }

        private static IEnumerable<SyntaxTree> GetActionSyntaxTreeList(CompileConfiguration cc, string actionsFileText)
        {
            if (actionsFileText == null)
            {
                actionsFileText = File.ReadAllText(cc.actionsOutputFilename);
            }
            var treeList = new List<SyntaxTree>() { CSharpSyntaxTree.ParseText(actionsFileText, null, cc.actionsFilename) };
            if (cc.externalActionsExtensionOutputFilename != null)
            {
                var externalActionsOutputFileText = File.ReadAllText(cc.externalActionsExtensionOutputFilename);
                treeList.Add(CSharpSyntaxTree.ParseText(externalActionsOutputFileText, null, cc.externalActionsExtensionOutputFilename));
                if (cc.externalActionsExtensionFilename != null)
                {
                    var externalActionsFileText = File.ReadAllText(cc.externalActionsExtensionFilename);
                    treeList.Add(CSharpSyntaxTree.ParseText(externalActionsFileText, null, cc.externalActionsExtensionFilename));
                }
            }
            return treeList;
        }
        ...
        private bool ProcessModel(CompileConfiguration cc)
        {
            ...
            List<MetadataReference> compRefs;
            CSharpCompilationOptions compOpts;
            SetupCoreCompiler(null, out compOpts, out compRefs);
            string outputAssemblyName = cc.destDir + "lgsp-" + modelName;
            try
            {
                var compilation = CSharpCompilation.Create(outputAssemblyName, GetModelSyntaxTreeList(cc), compRefs, compOpts);
                var result = compilation.Emit(cc.destDir + "lgsp-" + modelName + ".dll");
                if (!result.Success)
                {
                    ConsoleUI.errorOutWriter.WriteLine("Illegal model C# source code: " + result.Diagnostics.Count() + " Errors:");
                    foreach (Diagnostic error in result.Diagnostics)
                    {
                        ConsoleUI.errorOutWriter.WriteLine(error.Location.GetLineSpan().Path + "  " + error.Location.GetLineSpan().ToString() + ": " + error.GetMessage());
                    }
                    return false;
                }
            }
            ...
            cc.modelAssembly = AssemblyLoadContext.Default.LoadFromAssemblyPath(Path.GetFullPath(outputAssemblyName+".dll"));
            cc.modelAssemblyName = outputAssemblyName;
        ...
        private static void SetupCoreCompiler(String modelAssemblyName,
            out CSharpCompilationOptions compOptions, out List<MetadataReference> compReferences)
        {
            compOptions = new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary)
                .WithOptimizationLevel(OptimizationLevel.Release)
                .WithUsings(new[] { "System" });
            compReferences = new List<MetadataReference>() {
                MetadataReference.CreateFromFile(Path.Combine(Path.GetDirectoryName(typeof(System.Runtime.GCSettings).Assembly.Location)!, "System.Runtime.dll")),
                MetadataReference.CreateFromFile(typeof(Object).Assembly.Location),
                MetadataReference.CreateFromFile(typeof(IBackend).Assembly.Location),
                MetadataReference.CreateFromFile(typeof(LGSPAction).Assembly.Location)
            };
            if (modelAssemblyName != null)
            {
                compReferences.Add(MetadataReference.CreateFromFile(modelAssemblyName + ".dll"));
            }
        }
            ...
            out Assembly initialAssembly)
        {
            ...
            SetupCoreCompiler(modelAssemblyName, out CSharpCompilationOptions compOpts, out List<MetadataReference> compRefs);
            compRefs.Add(MetadataReference.CreateFromFile(Path.GetDirectoryName(typeof(List<int>).Assembly.Location) + Path.DirectorySeparatorChar + "System.Collections.dll"));
            initialAssembly = null;
            try
            {
                var actionName = Path.GetFileName(actionsFilename).Split(".")[0];
                var compilation = CSharpCompilation.Create(actionName, GetIntermediateSyntaxTreeList(actionsFilename), compRefs, compOpts);
                ...
                initialAssembly = AssemblyLoadContext.Default.LoadFromStream(stream);
                return ErrorType.NoError;
            }
            catch (Exception ex)
            {
        ...
        private ErrorType CompileActions(CompileConfiguration cc, String actionsOutputSource)
        {
            if (true)
                {
                SetupCoreCompiler(cc.modelAssemblyName, out CSharpCompilationOptions compOpts, out List<MetadataReference> compRefs);
                //TODO: test this
                foreach (var assembly in cc.externalAssemblies) 
                {
                    compRefs.Add(MetadataReference.CreateFromFile(assembly));
                }
                compRefs.Add(MetadataReference.CreateFromFile(Path.GetDirectoryName(typeof(List<int>).Assembly.Location) + Path.DirectorySeparatorChar + "System.Collections.dll"));
                try
                {
                    var actionName = Path.GetFileName(cc.actionsOutputFilename).Split(".")[0];
                    CSharpCompilation compilation;
                    if ((flags & ProcessSpecFlags.KeepGeneratedFiles) != 0)
                    {
                        compilation = CSharpCompilation.Create(actionName, GetActionSyntaxTreeList(cc, null), compRefs, compOpts);
                    }
                    else
                    {
                        compilation = CSharpCompilation.Create(actionName, GetActionSyntaxTreeList(cc, actionsOutputSource), compRefs, compOpts);
                    }
                        
                    var result = compilation.Emit(cc.destDir + "lgsp-" + cc.actionsName + ".dll");
                    if (!result.Success)
                    {
                        ConsoleUI.errorOutWriter.WriteLine("Illegal model C# source code: " + result.Diagnostics.Count() + " Errors:");
                        foreach (Diagnostic error in result.Diagnostics)
                ...
                {
                    ConsoleUI.errorOutWriter.WriteLine("Unable to compile actions: {0}", ex.Message);
                    return ErrorType.GrGenNetError;
                }
            }
        }
